...
# 5. Find the Shape of Our Dataset (Rows and COlumns)
if upload is not None:
    data_shape = st.radio(
...
if upload is not None:
    ...
    if test == True:
        if st.checkbox("Null Values is the Dataset"):
...
if upload is not None:
    ...
    if test == True:
        ...
        dup = st.selectbox(
            "Do You Want to Remove Duplicate Values?", ("Select One", "Yes", "No"))
        if dup == "Yes":
            data = data.drop_duplicates()
            st.text("Duplicate Values are Removed")
        if dup == "No":
            st.text("Okay, No Problematics")
